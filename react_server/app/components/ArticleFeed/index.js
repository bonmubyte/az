...
export default class ArticleFeed extends React.Component {
	...
	fetchComments = () => {
		axios.get(`/api/get_saved_comments_by_post/?post=${this.props.feed.post_id}`).then((response) => {
			this.setState({
	...
	getTotalRepliesCount = () => {
		axios.get(`/api/get_total_commments_count/?post=${this.props.feed.post_id}&kind=3`).then((response) => {
			this.setState({ totalReplies: response.data.total });
		});
	};
	componentDidMount() {
		this.setState({
	...
	removeComment = async () => {
		await deletePost(this.props.feed.post_id);
		this.props.refresh();
	};
	...
	pin = async () => {
		...
		data.append('id', this.props.feed.post_id);
		await axios.post('/api/pin_post/', data);
		this.props.refresh();
	};
	publishComment = async () => {
		...
		user = JSON.parse(user);
		const userID = get(user, 'id', null);
		const cmnt = this.state.commentField;
		let data = new FormData();
		data.append('user', userID);
		data.append('post', this.props.feed.post_id);
		data.append('comment', cmnt);
		data.append('kind', 3);
		await axios.post('/api/add_comment/', data);
	...
	commentaryMenu = () => {
		...
		if (window.location.href.split('/')[window.location.href.split('/').length - 1].split('#')[0] == currentUser) {
			content = (
				<Menu>
					<Menu.Item>
						<div
						...
						>
							<p style={{ cursor: 'pointer' }}>
								{this.props.feed.pinned == 'pinned' ? 'Unpin the commentary' : 'Pin the commentary'}
							</p>
						</div>
					</Menu.Item>
					<Menu.Item>
	...
	render() {
		return (
			<Row className={'feed-card'} id={`article_${this.props.feed.post_id}`}>
				<Row type="flex" span={24}>
					...
					<Col className={'feed-user'}>
						<Link to={`/news-page/${this.props.feed.creator_id}`}>&nbsp; {this.props.feed.user_name}</Link>
					</Col>

					<Col className={'feed-group'}>
						<Dropdown trigger={[ 'click' ]} overlay={this.commentaryMenu()}>
						...
						<Row>
							...
							<Col className={'post-content'}>
								...
								<Row className={'post-info'}>
									<span className={'post-author'}>{this.props.feed.post_author}</span>
									&nbsp;&nbsp;
				...
				<Row style={{ marginTop: 10, paddingTop: 10, borderTop: '1px solid gray' }} type="flex">
					<Col style={{ marginTop: 10, marginLeft: 30, display: 'flex' }}>
						<div onClick={() => this.voteFeed()} style={{ marginRight: 20 }}>
							<img
					...
					<Col style={{ marginLeft: 30, display: 'flex' }}>
						<img
							style={{ width: 54, height: 46, cursor: 'pointer', marginRight: 20 }}
				...
				{this.state.commentActive ? (
					<React.Fragment>
						<Row>
							...
							<Col style={{ cursor: 'pointer', fontSize: '.8em' }} offset={1} span={4}>
