...
class CustomUser(AbstractUser):
    first_mode = models.BooleanField(blank=False, default=False)

class UserManager(BaseUserManager):

    use_in_migrations = True

    def create_user(self, email, username, first_mode, password=None):
        user = self.model(
    ...
    def create_staffuser(self, email, username, first_mode, password):
...
class UserGroup(models.Model):
    ...
    creator = models.ForeignKey(CustomUser, on_delete=models.CASCADE, related_name='usergroup_creator')
    def __str__(self):
        return self.name
...
class Comment(models.Model):
    ...
    def get_info(self, request):
        ...
        profile_obj  = Profile.objects.filter(user=self.user).first()
        if profile_obj:
            json_dic['current_profile'] = profile_obj.get_obj_dic(request)
        else:
            json_dic['current_profile'] = None
        json_dic['like_count'] = VoteComment.objects.filter(comment=self).count()
        json_dic['kind'] = self.kind
        if self.kind == 1:
            group = GroupComment.objects.filter(comment=self).first()
            if group:
               json_dic['url'] = '/group/{}#commentary_{}'.format(GroupComment.objects.filter(comment=self).first().group.id, self.id)
            else:
                json_dic['url'] = '/group/{}#commentary_{}'.format('', self.id)
        elif self.kind == 2:
